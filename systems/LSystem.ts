...
export default class LSystem {
  ...
  constructor(config: LSystemConfig = {}) {
    ...
    if (this.params) {
      // Initialize with CSystem style parameters if params are provided
      this.axiom = Array(this.params.axiomAmount).fill("[X]++").join("");
      this.startLength = this.params.startLength;
      this.theta = ((this.params.thetaDeg) * Math.PI) / 180;
      this.generateRules();
    } else {
  ...
  simulate(gen: number) {
    if (this.params) {
      this.simulateComplex(gen);
    } else {
      this.simulateSimple(gen);
    }
  }

  private simulateSimple(gen: number) {
  ...
  private simulateComplex(gen: number) {
    while (this.generations < gen) {
      let nextProd = "";
