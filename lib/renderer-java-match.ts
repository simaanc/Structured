...
export const hsbToRgb = (h: number, s: number, b: number): [number, number, number] => {
  ...
  let r, g, b1;
  switch (i % 6) {
    case 0: r = bNorm; g = t; b1 = p; break;
    case 1: r = q; g = bNorm; b1 = p; break;
    case 2: r = p; g = bNorm; b1 = t; break;
    case 3: r = p; g = q; b1 = bNorm; break;
    case 4: r = t; g = p; b1 = bNorm; break;
    case 5: r = bNorm; g = p; b1 = q; break;
...
export function renderJavaMatch(
  ...
  for (let i = 0; i < steps; i++) {
    ...
    const currentW = Math.random() * 
      (height * params.maxSizeMultiplier - height * params.minSizeMultiplier) + 
      height * params.minSizeMultiplier;
    
    const currentH = (params.heightRatio >= 0 && params.widthRatio >= 0) 
      ...
      case 'F':
        ...
        for (let j = 0; j < repeats; j++) {
          if (params.toggles.line) {
            ...
            ctx.lineTo(state.x + finalW * Math.cos(state.angle), 
          ...
          if (params.toggles.circle) {
            ctx.beginPath();
            ctx.ellipse(state.x + finalW/2, state.y + finalH/2, finalW/2, finalH/2, 0, 0, Math.PI * 2);
