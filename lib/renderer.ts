...
import { 
   ...
   */
  const drawTriangle = (ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number): void => {
    ctx.beginPath();
   ...
   * @param {number} h - Height
   */
  const drawHexagon = (ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number): void => {
   ...
   * @param {number} h - Height
   */
  const drawCube = (ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number): void => {
   ...
   */
  const lerpColorComponent = (current: number, lerpAmount: number, max: number): number => {
  ...
  const updateColor = (state: DrawingState, lerpFrequency: number): void => {
    state.randomR = lerpColorComponent(state.randomR, lerpFrequency, COLOR.MAX_HUE);
    state.randomG = lerpColorComponent(state.randomG, lerpFrequency, COLOR.MAX_SATURATION);
    state.randomB = lerpColorComponent(state.randomB, lerpFrequency, COLOR.MAX_BRIGHTNESS);
  };
  ...
  const drawShape = (
    ...
    switch (shapeType) {
      ...
      case ShapeType.SQUARE:
        if (toggles.square) {
          if (toggles.squareFill) {
            ctx.fillStyle = `rgba(${hsbToRgb(state.randomR, state.randomG, state.randomB).join(',')}, ${(params.alpha / 255) * (state.opacity / 100)})`;
            ctx.fillRect(state.x, state.y, w, h);
          }
          ctx.strokeRect(state.x, state.y, w, h);
      ...
      case ShapeType.CIRCLE:
        if (toggles.circle) {
          ctx.beginPath();
          ctx.ellipse(state.x, state.y, w/2, h/2, 0, 0, Math.PI * 2);
  ...
  /**
   * Calculate shape size based on parameters
   * @param {number} height - Canvas height
   * @param {SystemParams} params - Rendering parameters
   * @returns {{width: number, height: number}} - Width and height for shape
   */
  const calculateShapeSize = (height: number, params: SystemParams): { width: number, height: number } => {
    const { minSizeMultiplier, maxSizeMultiplier, widthRatio, heightRatio, size } = params;
    
    // Calculate base width with random multiplier in the specified range
    const currentW = Math.random() * 
      (height * maxSizeMultiplier - height * minSizeMultiplier) + 
      height * minSizeMultiplier;
    
    // Calculate height based on ratio settings
    const currentH = (heightRatio >= 0 && widthRatio >= 0) 
  ...
  /**
   * Render an L-system on a canvas
  ...
  export const renderLSystem = (
    ...
    const { complexity, scatter, color } = params;
    ...
    const operations: Operation[] = typeof production === 'string' 
    ...
    const operationActions: Record<string, (value?: number) => void> = {
      [OperationType.ROTATE_RIGHT]: () => { state.angle += theta * repeats; },
      [OperationType.ROTATE_LEFT]: () => { state.angle -= theta * repeats; },
      [OperationType.ROTATE_RIGHT_DOUBLE]: () => { state.angle += theta * 2 * repeats; },
      [OperationType.ROTATE_RIGHT_HALF]: () => { state.angle += (theta / 2) * repeats; },
      [OperationType.ROTATE_SQUARED]: () => { state.angle += (theta * theta) * repeats; },
      [OperationType.ROTATE_CUBED]: () => { state.angle += (theta * theta * theta) * repeats; },
      [OperationType.PUSH_STATE]: () => { stack.push({...state}); },
      [OperationType.POP_STATE]: () => { if (stack.length > 0) Object.assign(state, stack.pop()); },
      [OperationType.DRAW_FORWARD]: () => {
        // Calculate shape dimensions
        const { width: shapeWidth, height: shapeHeight } = calculateShapeSize(height, params);
      ...
      [OperationType.REPEAT]: (value?: number) => { if (value !== undefined) repeats = value; }
    };
    ...
    for (let i = 0; i < steps; i++) {
      ...
      if (operation.type === OperationType.REPEAT && operation.value !== undefined) {
        ...
        if (operation.type !== OperationType.PUSH_STATE && 
